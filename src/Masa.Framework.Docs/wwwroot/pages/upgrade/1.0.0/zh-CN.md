...
2.  FluentValidation扩展验证 (包名: `Masa.Utils.Extensions.Validations.FluentValidation`)
    ...
    1. <font color=Red>FluentValidation扩展验证器</font>都修改为<font color=Red>Null</font>值默认<font color=Red>不</font>再进行<font color=Red>校验</font>。[PR #485](https://github.com/masastack/MASA.Framework/pull/485), 必填的字段，需要加入`NotNull`校验
      ...
      > Masa定义的Validator扩展默认逻辑与`FluentValidation`官方的内置Validator逻辑不一致，为了保持一致，验证器扩展默认不对Null值进行校验, 受影响的`PhoneValidator`、`IdCardValidator`
...
4. 隔离性调整
    ...
    * 用法调整
        ...
        builder.Services
            .AddDomainEventBus(dispatcherOptions =>
            {
                dispatcherOptions.UseIntegrationEventBus<IntegrationEventLogService>(options => options.UseDapr().UseEventLog<CustomDbContext>())
                    ...
                    .UseIsolationUoW<CustomDbContext>(
                        isolationBuilder => isolationBuilder.UseMultiTenant(),
                        dbOptions => dbOptions.UseSqlServer("更换SqlServer数据库地址"))
        ...
        builder.Services
            ...
            {
                dbContext.UseSqlServer("更换SqlServer数据库地址"));
            })
        ...
        :::
        ::::
    > 调整后的隔离性与DDD无关, 通过引用`Masa.Contrib.Isolation.MultiTenant`、`Masa.Contrib.Data.EFCore.SqlServer`即可实现多租户

## 功能

1. FluentValidation扩展验证 (包名: `Masa.Utils.Extensions.Validations.FluentValidation`)
...
2. <font color=Red>MasaDbContext</font> 新增支持 <font color=Red>无参构造函数</font>
    ...
    > 相比注册MasaDbContext时使用数据库，新语法会有一些特别要注意的地方，详细可查看[文档](/framework/building-blocks/data/orm-efcore)
